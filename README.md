...
- **Infra stack** (`infra/terraform/`): Provisions all Azure resources. The base WAF policy is associated to the AFD endpoint default domain with `/*`, which is the path pattern supported by AFD security policy associations. WAF policies are created bare (no rules). `lifecycle { ignore_changes = [managed_rule, custom_rule, mode, enabled] }` ensures that config-stack rule and mode changes are never reverted by an infra apply.
- **Config stack** (`infra/terraform-config/`): Imports the WAF policies by ID and applies `managed_rule` blocks built from JSON. `config/waf/api-policies.json` declares APIM API bindings and candidate API-specific WAF policy packages. AFD does not support path-scoped WAF policy associations on the endpoint default domain, so API-specific packages are not active unless you later expose APIs through separate domains/endpoints that can carry separate `/*` WAF associations.
...
To add another API-specific policy package, add it to `config/waf/api-policies.json` with its APIM API name, then optionally add `config/waf/{env}/apis/{api}/exclusions.json` and `rule-overrides.json` for API-specific tuning. API keys must be lowercase letters, numbers, or hyphens, and the APIM API name must exist in Terraform. API packages inherit `config/waf/base/`; use `disabledBaseExclusions` in the API-specific `exclusions.json` when previewing an API opt-out from one inherited base exclusion, such as API1 disallowing `$top` while API2 still allows it. These API-specific WAF policies require separate Front Door domains/endpoints to become active.

Validate WAF tuning before opening a pull request with `scripts/test-waf-config.ps1`. Preview effective WAF tuning before deployment with `scripts/show-effective-waf-config.ps1 -Environment dev`. The preview script shows the merged base, environment, and API-specific policy packages, including inherited exclusions that are disabled for a specific API package. Add `-AsJson` for machine-readable output.
