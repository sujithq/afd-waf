...
- **Config stack** (`infra/terraform-config/`): Imports the WAF policies by ID and applies `managed_rule` blocks built from JSON. `config/waf/api-policies.json` declares which APIM APIs get API-specific WAF policies and any inherited base exclusions they disable. API1 protects `/odata1/*`; API2 protects `/odata2/*` in this demo. The infra stack derives those AFD route patterns from APIM API paths and creates matching WAF associations.
...
To add another API-specific WAF policy, add it to `config/waf/api-policies.json` with its APIM API name, then optionally add `config/waf/{env}/apis/{api}/exclusions.json` and `rule-overrides.json` for API-specific tuning. API keys must be lowercase letters, numbers, or hyphens, and the APIM API name must exist in Terraform. Terraform derives the AFD route and WAF association path from the APIM API path. API policies inherit `config/waf/base/`; use `disabledBaseExclusions` in the API-specific `exclusions.json` when that API must opt out of one inherited base exclusion, such as API1 disallowing `$top` while API2 still allows it.
