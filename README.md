...
- **Infra stack** (`infra/terraform/`): Provisions all Azure resources. The base WAF policy is associated to the AFD endpoint default domain with `/*`, which is the path pattern supported by AFD security policy associations. Stage 1 also creates disabled domain-policy WAF resources and APIM routes for the future custom-domain demo, but it does not create DNS-dependent AFD custom domains yet. WAF policies are created bare (no rules). `lifecycle { ignore_changes = [managed_rule, custom_rule, mode, enabled] }` ensures that config-stack rule and mode changes are never reverted by an infra apply.
